...
def convert_from_cls_format(cls_boxes, cls_depths, cls_segms, cls_keyps):
    ...
    return boxes, depths, segms, keyps, classes
...
def vis_one_image(
        im, im_name, output_dir, boxes, depths=None, segms=None, keypoints=None, 
        thresh=0.9, kp_thresh=2, dpi=200, box_alpha=0.0, dataset=None, 
        show_class=False, show_depth=True, ext='pdf'):
    """Visual debugging of detections."""
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    if isinstance(boxes, list):
        boxes, depths, segms, keypoints, classes = convert_from_cls_format(
            boxes, depths, segms, keypoints)
    ...
    for i in sorted_inds:
        ...
        if depths is not None and show_depth:
            depth = depths[i]
            ax.text(
